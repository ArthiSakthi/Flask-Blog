...
con.execute(
    "create table if not exists users(pid integer primary key,name text,password text,contact integer,mail text)")
con.close()
db = SQLAlchemy(app)
...
my_admin = Admin(app, name='Blog Page', template_mode='bootstrap3', index_view=CustomAdminIndexView())
